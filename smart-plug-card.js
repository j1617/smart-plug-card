...
class SmartPlugCard extends HTMLElement {
  ...
  setConfig(config) {
    this.config = {
      ...
      title: '智能插座',
      // 实体配置
      entities: config.entities || [],  // 支持多个插座，每个元素可以是字符串或对象
      // 单个插座配置（兼容）
      ...
      indicator_on_color: '#22c55e',
      indicator_off_color: '#ef4444',
      ...config
    };
    this._updateCard();
  }
  ...
  _getValue(state, attribute) {
    if (!state) return null;
    if (attribute) {
      return state.attributes[attribute];
    }
    const val = state.state;
    if (val === 'unavailable' || val === 'unknown') return null;
    return val;
  }
  ...
  _buildPlugItem(entityConfig) {
    // entityConfig 可以是字符串（switch实体ID）或对象
    ...
    if (typeof entityConfig === 'object') {
      ...
      if (entityConfig.indicator_entity) {
        const indicatorState = this._getState(entityConfig.indicator_entity);
        var indicatorOn = indicatorState?.state === 'on';
      }
    } else {
      // 单个插座模式：从根配置读取
  ...
  _createPlugItem(entity) {
    const { is_on, name, power, voltage, current, daily, monthly, indicator_on } = entity;
    const statusColor = is_on ? '#22c55e' : '#ef4444';
    const statusText = is_on ? '已开启' : '已关闭';
    const icon = is_on ? '🟢' : '⚪';

    // 格式化数值
    const powerDisplay = power !== null && power !== undefined ? `${this._formatNumber(power, 0)} W` : '--';
    const voltageDisplay = voltage !== null && voltage !== undefined ? `${this._formatNumber(voltage, 1)} V` : '--';
    const currentDisplay = current !== null && current !== undefined ? `${this._formatNumber(current, 2)} A` : '--';
    const dailyDisplay = daily !== null && daily !== undefined ? `${this._formatEnergy(daily)} kWh` : '--';
    const monthlyDisplay = monthly !== null && monthly !== undefined ? `${this._formatEnergy(monthly)} kWh` : '--';
    ...
    if (indicator_on !== undefined) {
      ...
      indicatorHtml = `
        <div class="plug-indicator">
          <span class="indicator-dot" style="background: ${indColor};"></span>
          <span class="indicator-text" style="color: ${indColor};">指示灯${indText}</span>
        </div>
      `;
    }
      ...
      <div class="plug-item">
        ...
        <div class="plug-metrics">
          <div class="metric-item">
            ...
            <span class="metric-value">${powerDisplay}</span>
            <span class="metric-label">功率</span>
          </div>
          <div class="metric-item">
            <span class="metric-icon">🔌</span>
            <span class="metric-value">${voltageDisplay}</span>
            <span class="metric-label">电压</span>
          </div>
          <div class="metric-item">
            <span class="metric-icon">📊</span>
            <span class="metric-value">${currentDisplay}</span>
        ...
        <div class="plug-energy">
          <div class="energy-item">
            ...
            <span class="energy-value">${dailyDisplay}</span>
          </div>
          <div class="energy-item">
            <span class="energy-label">本月用电</span>
            <span class="energy-value">${monthlyDisplay}</span>
          </div>
        </div>
      </div>
    `;
  ...
  _updateCard() {
    ...
    const statsHtml = totalPlugs > 0 ? `
      <div class="stats-bar">
        <div class="stat-item stat-on">
          <span class="stat-dot"></span>
          <span class="stat-label">开启</span>
  ...
  static getStubConfig() {
    return {
      ...
      power_entity: 'sensor.my_plug_power',
      voltage_entity: 'sensor.my_plug_voltage',
      current_entity: 'sensor.my_plug_current',
      daily_energy_entity: 'sensor.my_plug_daily_energy',
      monthly_energy_entity: 'sensor.my_plug_monthly_energy'
    };
  }
}
